...
if(!empty($emod_r[$mid]['listandf'])&&$_GET['ph']==1)
{
	...
	for($i=1;$i<$count;$i++)
	{
		...
		if(!empty($andval))
		{
			...
			if(empty($andval)){$andval=0;}else{$andval=$andval;}
			$search2.="$andval".'-';
	}
	if($listandf)
	{
...
if($orderby=='onclick'){$orderby=1;}else{$orderby=0;}
$search2.=$orderby;
$page=(int)$_GET['page'];
...
$pagejiehes=pagejiehe($num,$line,$page_line,$start,$page,$search2);
$pagejiehes2=pagejiehe2($num,$line,$page_line,$start,$page,$search2);
//页面支持标签
if($public_r['dtcanbq'])
{
...
$listtemp=str_replace('[!--show.pagejiehes--]',$pagejiehes,$listtemp);
$listtemp=str_replace('[!--show.pagejiehes2--]',$pagejiehes2,$listtemp);
//取得列表模板
