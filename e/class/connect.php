...
//网游结合项分页
function pagejiehe($num,$line,$page_line,$start,$page,$search){
	global $fun_r;
	if($num<=$line)
	{
		return '';
	...
	$snum=2;//最小页数
	$totalpage=ceil($num/$line);//取得总页数
	$firststr='<span class="a1 disabled">共 '.$totalpage.' 页</span>';
	//上一页
	if($page<>0)
	{
		$toppage='<a href="/list-'.$search.'-0-0.html'.'">'.$fun_r['startpage'].'</a>';
		$pagepr=$page-1;
		$prepage='<a href="/list-'.$search.'-'.$pagepr.'-0.html'.'">'.$fun_r['pripage'].'</a>';
	}
	//下一页
	if($page!=$totalpage-1)
	{
		$pagenex=$page+1;
		$nextpage='<a href="/list-'.$search.'-'.$pagenex.'-0.html'.'">'.$fun_r['nextpage'].'</a>';
		$lastpage='<a href="/list-'.$search.'-'.($totalpage-1).'-0.html'.'">'.$fun_r['lastpage'].'</a>';
	}
	$starti=$page-$snum<0?0:$page-$snum;
	$no=0;
	for($i=$starti;$i<$totalpage&&$no<$page_line;$i++)
	{
		...
		{
			$is_1='<a href="/list-'.$search.'-'.$i.'-0.html'.'">';
			$is_2="</a>";
		}
		$pagenum=$i+1;
		$returnstr.=$is_1.$pagenum.$is_2;
	}
	$returnstr=$firststr.$prepage.$returnstr.$nextpage;
	return $returnstr;
}


//小游戏结合项分页
function pagejiehe2($num,$line,$page_line,$start,$page,$search){
	global $fun_r;
	if($num<=$line)
	{
		return '';
	...
	if($page<>0)
	{
		$toppage='<a href="/list-'.$search.'-0-1.html'.'">'.$fun_r['startpage'].'</a>';
		$pagepr=$page-1;
		$prepage='<a href="/list-'.$search.'-'.$pagepr.'-1.html'.'">'.$fun_r['pripage'].'</a>';
	}
	//下一页
	if($page!=$totalpage-1)
	{
		$pagenex=$page+1;
		$nextpage='<a href="/list-'.$search.'-'.$pagenex.'-1.html'.'">'.$fun_r['nextpage'].'</a>';
		$lastpage='<a href="/list-'.$search.'-'.($totalpage-1).'-1.html'.'">'.$fun_r['lastpage'].'</a>';
	}
	$starti=$page-$snum<0?0:$page-$snum;
	$no=0;
	for($i=$starti;$i<$totalpage&&$no<$page_line;$i++)
	{
		...
		{
			$is_1='<a href="/list-'.$search.'-'.$i.'-1.html'.'">';
			$is_2="</a>";
		}
		$pagenum=$i+1;
		$returnstr.=$is_1.$pagenum.$is_2;
