...
<a onclick="window.open('ecmseditor/FileMain.php?type=1&classid=<?=$classid?>&infoid=<?=$id?>&filepass=<?=$filepass?>&sinfo=1&doing=1&field=titlepic<?=$ecms_hashur[ehref]?>','','width=700,height=550,scrollbars=yes');" title="选择已上传的图片"><img src="../data/images/changeimg.gif" border="0" align="absbottom"></a></td></tr><tr><td width='16%' height=25 bgcolor='ffffff'>分类</td><td bgcolor='ffffff'><select name="fenlei" id="fenlei"><option value="1"<?=$r[fenlei]=="1"?' selected':''?>>卡牌游戏</option><option value="2"<?=$r[fenlei]=="2"?' selected':''?>>角色扮演</option><option value="3"<?=$r[fenlei]=="3"?' selected':''?>>模拟经营</option><option value="4"<?=$r[fenlei]=="4"?' selected':''?>>射击游戏</option><option value="5"<?=$r[fenlei]=="5"?' selected':''?>>动作游戏</option><option value="6"<?=$r[fenlei]=="6"?' selected':''?>>战争策略</option><option value="7"<?=$r[fenlei]=="7"?' selected':''?>>棋牌游戏</option><option value="8"<?=$r[fenlei]=="8"?' selected':''?>>休闲游戏</option></select></td></tr><tr><td width='16%' height=25 bgcolor='ffffff'>一句话</td><td bgcolor='ffffff'>
