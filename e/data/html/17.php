...
<a onclick="window.open('ecmseditor/FileMain.php?type=1&classid=<?=$classid?>&infoid=<?=$id?>&filepass=<?=$filepass?>&sinfo=1&doing=1&field=titlepic<?=$ecms_hashur[ehref]?>','','width=700,height=550,scrollbars=yes');" title="选择已上传的图片"><img src="../data/images/changeimg.gif" border="0" align="absbottom"></a></td></tr><tr><td width='16%' height=25 bgcolor='ffffff'>分类</td><td bgcolor='ffffff'><select name="fenlei" id="fenlei"><option value="1"<?=$r[fenlei]=="1"?' selected':''?>>五子棋</option><option value="2"<?=$r[fenlei]=="2"?' selected':''?>>象棋</option><option value="3"<?=$r[fenlei]=="3"?' selected':''?>>围棋</option><option value="4"<?=$r[fenlei]=="4"?' selected':''?>>飞行棋</option><option value="5"<?=$r[fenlei]=="5"?' selected':''?>>中国暗棋</option><option value="6"<?=$r[fenlei]=="6"?' selected':''?>>跳棋</option><option value="7"<?=$r[fenlei]=="7"?' selected':''?>>井字棋</option><option value="8"<?=$r[fenlei]=="8"?' selected':''?>>军棋</option><option value="9"<?=$r[fenlei]=="9"?' selected':''?>>斗兽棋</option><option value="10"<?=$r[fenlei]=="10"?' selected':''?>>将棋</option><option value="11"<?=$r[fenlei]=="11"?' selected':''?>>四子棋</option><option value="12"<?=$r[fenlei]=="12"?' selected':''?>>大富翁</option><option value="13"<?=$r[fenlei]=="13"?' selected':''?>>国际象棋</option></select></td></tr><tr><td width='16%' height=25 bgcolor='ffffff'>一句话</td><td bgcolor='ffffff'>
