...
$num=$empire->gettotal("select count(*) as total from www_96kaifa_com_ecms_libao  where title like '%$word%'");
?>
<!doctype html>
<!DOCTYPE html>
<html lang="zh-CN">
...
<?php }else{
$Page_size=20; 
$result=$empire->query("select * from www_96kaifa_com_ecms_libao where title like '%$word%'");
...
$sql=$empire->query("select * from www_96kaifa_com_ecms_libao where title like '%$word%' order by newstime desc limit $offset,$Page_size");

while ($row=$empire->fetch($sql)) { 

if($row[kstime]=="0000-00-00" && $row[jstime]=="0000-00-00"){ $bian='<p>永久有效</p>';}else{ $bian='<p>有效期：'.date("Y-m-d",strtotime($row[kstime])).' 至 '.date("Y-m-d",strtotime($row[jstime])).'</p>';} 
$cpl=round(($row[yiling]/$row[zongshu])*100,2)*100; $ss=100-$cpl;

?> 		
			<li>
			  <div class="img"><a href="<?php echo $row['titleurl']?>" target="_blank"><img src="<?php echo $row['titlepic']?>"></a></div>
			  ...
			  <div class="lq-btn"><a href="<?php echo $row['titleurl']?>" target="_blank" class="lqBtn">领取</a></div>
			</li>
<?php 
} 
if($count==0){
...
if($page!=1){ 
//$key.="<a href=\"/search/libao.php?key=".$word."&page=1"."\" class=\"a1\">首页</a> "; //首页 
$key.="<a href=\"/search/libao.php?key=".$word."&page=".($page-1)."\" class=\"a1\">上一页</a>"; //上一页 
}else { 
$key.="";//首页 
$key.=""; //上一页 
} 
...
$key.=" <a href=\"/search/libao.php?key=".$word."&page=".$i."\" class=\"a1\">".$i."</a>"; 
} 
} 
if($page!=$pages){ 


$key.=" <a href=\"/search/libao.php?key=".$word."&page=".($page+1)."\" class=\"a1\">下一页</a> ";//下一页 
//$key.=" <a href=\"/search/libao.php?key=".$word."&page=".$pages."\" class=\"a1\">尾页</a>"; //尾页 
}else { 
$key.=" ";//下一页 
$key.=""; //尾页 
} 
